...
Small, runnable scripts demonstrating the Volante library API (see the main
[README](../README.md#usage) for the full CLI / Web UI / library overview).

- **`fake_provider.py`** — fully offline, no API key needed. Wires
  `volante.providers.fake.FakeProvider` into a `Runtime` by hand and runs one goal
...
- **`minimal_library.py`** — builds a runtime from your environment
  (`volante.build_providers_from_env` + `volante.make_runtime_factory`) and runs a real
