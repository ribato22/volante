...
uvx --from "volante[mcp]==0.4.1" volante-mcp

# or install it and run the console script:
pip install "volante[mcp]==0.4.1"   # then:
volante-mcp
...
```bash
claude mcp add volante -- uvx --from "volante[mcp]==0.4.1" volante-mcp
```
...
```jsonc
{
  "mcpServers": {
    "volante": {
      "command": "uvx",
      "args": ["--from", "volante[mcp]==0.4.1", "volante-mcp"]
    }
  }
}
```
...
- **OpenAI Codex CLI** — `codex mcp add volante -- uvx --from "volante[mcp]==0.4.1" volante-mcp`
  (writes an `[mcp_servers.volante]` block to `~/.codex/config.toml`; add providers with repeated
  `--env KEY=VALUE`).
- **Gemini CLI** — `gemini mcp add volante uvx -- --from "volante[mcp]==0.4.1" volante-mcp`
...
The bundle wraps `uvx --from "volante[mcp]==0.4.1" volante-mcp`, so it runs locally and your
