# src/volante/providers/openai_compat.py
...
# chat.completions finish_reason -> canonical stop_reason.
...
def _canonical_stop_reason(finish_reason: object) -> str:
    """Map known wire reasons without disguising an incomplete/unknown stop."""
...
def _to_provider_error(err: BaseException, billing: str) -> ProviderError:
    ...
    if is_candidate_unavailable(msg, status=status):
    ...
    return ProviderError(
        ...
        retryable=retryable,
        status=status,
        provider_unavailable=status in {401, 403, 404},
    )
...
class OpenAICompatProvider:
    ...
    def __init__(
        ...
        *,
        ...
        # Retry and reroute are explicit Volante policies. Compatible SDK retries
        # would otherwise multiply physical calls behind quota/cost accounting.
    ...
    async def complete(self, req: CanonicalRequest) -> CanonicalResponse:
        ...
        stop_reason = _canonical_stop_reason(choice.finish_reason)
    ...
    async def stream(self, req: CanonicalRequest, on_text) -> CanonicalResponse:
        ...
        if not content:
            content = [TextBlock(text="")]
        stop_reason = (
            "early_stop" if stopped else _canonical_stop_reason(finish_reason)
        )
