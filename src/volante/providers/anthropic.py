...
def _to_provider_error(exc: Exception, billing: str = "card") -> ProviderError:
    ...
    if is_candidate_unavailable(msg, status=istatus):
    ...
    if istatus is not None:
        return ProviderError(
            msg,
            retryable=_retryable_status(istatus),
            status=istatus,
            provider_unavailable=istatus in {401, 403, 404},
        )
...
def _extract_stop_reason(resp: Any) -> str:
    """Preserve provider terminal reasons; missing/invalid means incomplete."""
...
class AnthropicProvider:
    ...
    def __init__(
        ...
        *,
        ...
        # Volante owns retry/reroute policy at the orchestration layer. Disable the
        # SDK's hidden retries so one logical attempt cannot multiply physical
        # calls, latency, cost, or subscription-budget accounting.
    ...
    async def complete(self, req: CanonicalRequest) -> CanonicalResponse:
        ...
        return CanonicalResponse(
            ...
            stop_reason=_extract_stop_reason(resp),
    ...
    async def stream(self, req: CanonicalRequest, on_text) -> CanonicalResponse:
        ...
        return CanonicalResponse(
            ...
            stop_reason=_extract_stop_reason(final),
            latency_ms=latency_ms,
        )
