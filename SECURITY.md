...
| `CODEX_ENABLED=1` (subscription Codex) | Isolated from your Codex config (`--ephemeral`, `--ignore-user-config`, `--ignore-rules`), API keys scrubbed, writes blocked (`--sandbox read-only`) | **Codex runs its own tools.** `read-only` is the policy for executing model-generated shell commands: it stops writes, not reads. This child can read anything your account can, outside `read_file`'s root. `CLAUDE_CODE_ENABLED=1` has no equivalent gap — that CLI is launched with its tools removed |
