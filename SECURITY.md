...
| Agentic `run_python`, `VOLANTE_SANDBOX=docker` | Real isolation: container, `--network none`, read-only root, `--cap-drop ALL`, `--pids-limit`, non-root user | Requires a trusted Docker daemon — and a trusted IMAGE: the default `python:3.12-slim` is a mutable tag, so a host without it pulls whatever that tag names at the time. Pass `image=` with a `@sha256:` digest, or pre-provision the image, if that boundary matters to you |
...
| `fetch_url` / `read_file` tools | Host-mediated: domain allowlist / root-confined path, no redirects, size cap, one end-to-end deadline | Prompt-injection containment holds **only** under the Docker sandbox |
| `CODEX_ENABLED=1` (subscription Codex) | Isolated from your Codex config (`--ephemeral`, `--ignore-user-config`, `--ignore-rules`), API keys scrubbed, writes blocked (`--sandbox read-only`) | **Codex runs its own tools.** `read-only` is the policy for executing model-generated shell commands: it stops writes, not reads. Verified live against codex-cli 0.145.0 — a prompt through this exact configuration read a file under `$HOME` and returned its contents, outside `read_file`'s root. `-c sandbox_permissions=[]` does not narrow it. `CLAUDE_CODE_ENABLED=1` has no equivalent gap — that CLI is launched with its tools removed |
| Eval scorer (`score_code`) | Forgery-resistant CHANNEL: process + filesystem separation + nonce-authenticated RPC — a solution cannot forge a result line, and must really compute to pass | Best-effort POSIX: `setsid()` escapes the `killpg` group (the wall-clock timeout still bounds it); not a sandbox for arbitrary hostile code; and since it is not, a solution can READ `eval/tasks.py` out of the repository and hard-code the expected answers. The channel is unforgeable; the benchmark is not cheat-proof |
